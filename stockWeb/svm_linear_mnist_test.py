...
def cost_func_kf(theta,*args):
	...
	cost=np.dot(e.T,e)+c_para*np.dot(theta.T,theta)
	#print(theta.shape)
	#print(X[idx].shape)
	#print(e.shape)
	grand=2*(np.dot(X[idx].T,e)+c_para*theta)
...
theta=10*np.random.random((X.shape[1],1))
...
#res=opt.fmin_tnc(cost_func_kf,x0=theta,args=(X,Y,c_para),approx_grad=False)

#print(theta.shape)

res=opt.fmin_l_bfgs_b(cost_func_kf,x0=theta,args=(X,Y,c_para),approx_grad=False)
